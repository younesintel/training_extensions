...
def debug_trace(func):
    ...
    def wrapped_function(self, *args, **kwargs):
        ...
        if self.task_environment.get_hyper_parameters().debug_parameters.enable_debug_dump:
            dump_dict = {
...
def export_debug_trace(self, export_type, output_model):
    return {"export_type": export_type}
...
def dump_dataset_item(item: DatasetItemEntity):
    dump = {"subset": item.subset, "numpy": item.numpy, "roi": item.roi, "annotation_scene": item.annotation_scene}
    return dump


def load_dataset_item(dump: Dict[str, Any]):
    return DatasetItemEntity(
        media=Image(dump["numpy"]), annotation_scene=dump["annotation_scene"], roi=dump["roi"], subset=dump["subset"]
    )


def dump_dataset(dataset: DatasetEntity):
    dump = {"purpose": dataset.purpose, "items": list(dump_dataset_item(item) for item in dataset)}
    return dump


def load_dataset(dump: Dict[str, Any]):
    return DatasetEntity(items=[load_dataset_item(i) for i in dump["items"]], purpose=dump["purpose"])
